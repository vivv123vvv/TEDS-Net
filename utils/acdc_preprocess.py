...
def resolve_acdc_root(raw_data_path):
    ...
    for candidate in candidates:
        training_dir = os.path.join(candidate, "training")
        testing_dir = os.path.join(candidate, "testing")
...
def _normalize_image(image_2d):
    min_value = float(image_2d.min())
    max_value = float(image_2d.max())
    if max_value > min_value:
        return ((image_2d - min_value) / (max_value - min_value)).astype(np.float32)
...
def preprocess_acdc_dataset(
    ...
    """将官方 ACDC NIfTI 预处理成训练与评估复用的 2D 缓存。"""
    ...
    for source_subset in ("training", "testing"):
        ...
        subset_output_dir = os.path.join(processed_root, source_subset)
        ...
        for patient_id in patient_dirs:
            ...
            for gt_path in gt_paths:
                ...
                for slice_index in range(image_volume.shape[-1]):
                    ...
                    relative_path = os.path.join(source_subset, slice_name).replace("\\", "/")
