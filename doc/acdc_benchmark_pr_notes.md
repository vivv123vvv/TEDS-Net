...
相较于原始 TEDS-Net 依赖 scaling-and-squaring 反复进行形变组合与采样的积分方式，当前实现以 R2Net / LC-ResNet 风格学习式积分器替代该环节，在保持 TEDS-Net 整体数据流与 prior warp 接口不变的前提下，更有利于降低速度与显存开销，也更便于围绕积分器本身控制变量开展同条件对照，并为后续多尺度或其他可学习积分器扩展提供统一接口。当前结果支持其作为更强实验基线，但对其是否严格优于原始 TEDS-Net 仍需同条件对照验证。
